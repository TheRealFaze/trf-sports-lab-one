...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Download Football-Data.co.uk league-seasons and normalize TRF match/odds CSVs."
    )
    ...
    with (
        ...
        ow.writerow([
            ...
            "home_odds", "draw_odds", "away_odds", "bookmaker", "snapshot_type"
        ])
        qw.writerow([
            "division", "start_year", "matches", "quotes", "closing_quotes",
            "fallback_quotes", "skipped_rows", "duplicate_matches"
        ])
        ...
        for year in args.years:
            for division in args.divisions:
                ...
                for m in season.matches:
                    mw.writerow([
                        m.date.isoformat(), division, m.home_team, m.away_team,
                        int(m.home_goals), int(m.away_goals)
                    ])
                for o in season.quotes:
                    ow.writerow([
                        o.date.isoformat(), division, o.home_team, o.away_team,
                        o.home_odds, o.draw_odds, o.away_odds, o.bookmaker, o.snapshot_type
                    ])
                q = season.quality
                qw.writerow([
                    division, year, q.matches, q.quotes, q.closing_quotes,
                    q.fallback_quotes, q.skipped_rows, q.duplicate_matches
                ])
