...
def test_football_data_parser_prefers_avg_closing():
    text = """Div,Date,HomeTeam,AwayTeam,FTHG,FTAG,B365H,B365D,B365A,AvgH,AvgD,AvgA,B365CH,B365CD,B365CA,AvgCH,AvgCD,AvgCA
E0,01/08/2025,Alpha,Beta,2,1,1.90,3.50,4.20,1.92,3.55,4.10,1.80,3.60,4.50,1.85,3.70,4.40
"""
    season = FootballDataUKClient().parse_season(text, division="E0", start_year=2025)
    assert len(season.matches) == 1
    assert len(season.quotes) == 1
    q = season.quotes[0]
    assert q.bookmaker == "FootballData Avg Close"
    assert q.snapshot_type == "CLOSE"
    assert q.home_odds == 1.85
    assert season.quality.closing_quotes == 1


def test_football_data_parser_marks_snapshot_fallback():
...
def test_odds_api_consensus_de_vigs_each_book_first():
    payload = {
        ...
        "data": [{
            "id": "evt1",
            "commence_time": "2025-08-01T19:00:00Z",
            "home_team": "Alpha",
            "away_team": "Beta",
            "bookmakers": [
